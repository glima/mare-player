...
oauth-open-url = Откройте этот URL в браузере:
oauth-enter-code = Или введите код: {$code}
waiting-for-login = Ожидание входа...
complete-login-in-browser = Завершите вход в браузере, затем вернитесь сюда.
...
cache = Кэш
song-cache = Кэш песен:
song-cache-size = {$used} МБ / {$max} МБ
song-cache-limit = Лимит кэша песен:
clear-cache = Очистить
account = Аккаунт
about = О программе
version = Версия
...
# Поделиться
share = Поделиться
share-description = Создать ссылку song.link и скопировать в буфер обмена
